...
const int inr1 = 8;
const int pwmr = 9;  // for 3.5 previusly 5 number pin there

const int STATE;
bool vac_on;
Servo myservo_magnet;  // create servo object to control a servo
...
const int pin_servo_magnet = 7; // for qualcome 9 is there
int key_value = 0;
...
int dir_value = 2;
int cmd;
int servo_brush_min_angle = 0;
int servo_brush_max_angle = 180;
unsigned long cmil;
...
void Cbservo_brush(const geometry_msgs::Twist& msg)
{
  servo_brush_min_angle = msg.linear.x;
  servo_brush_max_angle = msg.linear.y;
}
...
ros::Subscriber <geometry_msgs::Twist> sub_servo_brush("servo_brush_topic", Cbservo_brush );

void forward()
{
  myservo_magnet.write(servo_brush_max_angle);
...
void stopbase()
{
  ...
  val_dacl = 0;
  val_dacr = 0;
  myservo_magnet.write(servo_brush_min_angle);
  digitalWrite(inl1, LOW);
  analogWrite(pwml, 0);
...
void setup()
{
  ...
  nh.subscribe(sub_key_value);
  nh.subscribe(sub_servo_brush);
  myservo_magnet.attach(pin_servo_magnet);  // attaches the servo on pin 9 to the servo object
